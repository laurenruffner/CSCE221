...
template <typename T>
T MinQueue<T>:: min(){
    DListNode<T> *curr = this->dll.getFirst();
    DListNode<T> *small = this->dll.getFirst(); 
    int n = this->size(); 
    while (curr != &this->dll.trailer){
        if (curr->obj < small->obj){
            small = curr;
        }
        curr = curr->next;
    }
